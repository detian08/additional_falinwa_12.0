class fal_mattermost_integration {
    constructor(opts) {
        const s = this;
    ...
    buildSystray() {
        ...
        var IconMenu = s.odoo.Widget.extend({
            template: 'fal_mattermost_systray_integration',
        });
    ...
    setEvents() {
        const s = this;
        s.log(`setEvents`);
        // console.log(s.$('a.fal_mattermost_btn'));

        var myLoop = setInterval(function () {
            if (s.$('a.fal_mattermost_btn').length > 0) {
                s.$('a.fal_mattermost_btn').click(function (e) {
                    e.preventDefault();
    ...
    buildIframe(url) {
        ...
        style = style.join(';');
        var obj = '<iframe class="fal_mattermost_iframe" style="'+style+'" src="'+url+'" />';
        s.$(obj).appendTo('body');
    }
    ...
    toggleMailbox() {
        ...
        var iframe = s.$('.fal_mattermost_iframe');
        if (iframe.length === 0) {
            s.getAutoLoginUrl()
                .then(url => {
                    console.log(url)
                    s.buildIframe(url);
                    iframe = s.$('.fal_mattermost_iframe');
                    iframe.show();
                }).catch(e => {
...
odoo.define('fal_mattermost_integration', function (require) {
    "use strict";
    var lib = {
        RPC: require('web.rpc'),
    ...
    $(function () {
        let instance = new fal_mattermost_integration({
            version: "mm.0.004",
            debug: true,
            $: $,
            odoo: lib,
